...
int left_inner_threshold = 500;
int left_most_threshold = 200;
int right_inner_threshold = 300;
int right_most_threshold = 300;
volatile int counter;
int junction_on_line = 3;
  ...
  if (button_state == HIGH) {
    ...
    if(block_classified == HIGH){
      ...
      if(along_line == HIGH){
        ...
        if (((right_inner_value < right_inner_threshold && right_most_value < right_most_threshold && left_inner_value < left_inner_threshold) || (right_inner_value < right_inner_threshold && left_most_value < left_most_threshold && left_inner_value < left_inner_threshold)) &&((6 <= distance) && (distance <= 7))){
    ...
    if(sweep_done == HIGH){
      ...
      if (along_line == HIGH){
        ...
        if (distance<=5){   //Threshold height to trigger the block detector i.e. the ultrasonic sensor.
          ...
          detect_counter = 0;
          forwards();
          delay(2000); // Adjust this delay to ensure that the metal detector is in contact with the block.
          halt();
      ...
      // The assumption made is that the block will not be lost in the metal detection state.
        ...
        if(detect_counter>=20){ // Out of the reasonable distance.
          halt();
          LeftMotor->setSpeed(left_motor_speed);
          RightMotor->setSpeed(right_motor_speed);
          metal_state = LOW;
...
// The sencond major block, block sweeping. Work reasonably well.
      ...
      if (ready_to_sweep == LOW && first_sweep == HIGH){
       ...
       ready_to_sweep = HIGH;
        ...
        if(end_of_rotation == LOW){
          rotate_right();
          if (right_inner_value < left_inner_threshold){// The right inner sensor on the line.
    ...
    if(line_follow_state == HIGH){
     ...
     if (((right_inner_value < right_inner_threshold && right_most_value < right_most_threshold && left_inner_value < left_inner_threshold) || (right_inner_value < right_inner_threshold && left_most_value < left_most_threshold && left_inner_value < left_inner_threshold)) &&((6 <= distance) && (distance <= 7))) {
      ...
      if (junction_counter >= junction_on_line) {
        halt();
...
void line_follow(void){
    ...
    else if (left_most_value > left_most_threshold && right_most_value > right_most_threshold && left_inner_value < left_inner_threshold && right_inner_value < right_inner_threshold){
      forwards();
    }
