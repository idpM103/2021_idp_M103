...
void loop() {
  ...
  if (button_state == HIGH) {
    ...
    if(line_follow_state == HIGH){
     ...
     // On the junction
     if (left_most_value < left_threshold && left_inner_value < left_threshold && right_inner_value < right_threshold ) {
        //halt();
        junction_counter++;
        forwards();
        delay(1000);
      ...
      if (junction_counter >= 2) {// One junction is missing.
...
void line_follow(void){
    ...
    else if (left_most_value > left_threshold && left_inner_value > left_threshold && right_inner_value < right_threshold) {
      if (reverse_state == LOW){
        right();
        delay(50);
      }
