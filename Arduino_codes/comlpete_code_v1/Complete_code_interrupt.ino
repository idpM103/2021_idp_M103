  ...
  if (button_state == HIGH) {
    ...
    if(restore_flag == HIGH){
      if(end_of_rotation == LOW){
        rotate_right();
        if (right_inner_value < right_threshold){
        ...
        line_follow();
        if ((right_inner_value < right_threshold && right_most_value < right_threshold && left_inner_value < left_threshold) || (right_inner_value < right_threshold && left_most_value < left_threshold && left_inner_value < left_threshold)){ // On the junction to the main path.
          along_line = LOW;
          forwards();
          delay(100);
        }
    ...
    if(block_classified == HIGH){
      ...
      if(along_line == HIGH){
        ...
        if ((right_inner_value < right_threshold && right_most_value < right_threshold && left_inner_value < left_threshold) || (right_inner_value < right_threshold && left_most_value < left_threshold && left_inner_value < left_threshold)){
    ...
    if(sweep_done == HIGH){
      ...
      if (along_line == HIGH){
        ...
        else if (detect_counter >= 500000){ // The function for finding the lost block is still not ready.
      ...
      // The assumption made is that the block will not be lost in the metal detection state.
        ...
        if(detect_counter>=10){ // Out of the reasonable distance.
          // 此处不halt有机会掉线。
          metal_state = LOW;
...
void reset_wibich(){
  halt();
// Flags
  previous_button_value = LOW;
  current_button_value = LOW;
  button_state = LOW;        
  reverse_state = LOW;
...
void arm_drop(){
  ...
  arm_servo.attach(9);
  for (servo_pos = 45; servo_pos <= 180; servo_pos += 1) { // goes from 
...
void arm_raise(){
  ...
  arm_servo.attach(9);
  for (servo_pos = 180; servo_pos >= 45; servo_pos -= 1) { // 
    arm_servo.write(servo_pos);              // tell servo to go to position in variable 'pos'
    delay(15);          
  }             
  arm_servo.detach();
